...
portfolio_duration <- function(
  ...
  if (!is.null(col_portfolio) &&
      (!is.character(col_portfolio) ||
       length(col_portfolio) != 1L ||
       is.na(col_portfolio) ||
       !nzchar(col_portfolio))) {
  ...
  if (nrow(data_valid) > 0L) {
    ...
    bad_duration <- !is.finite(data_valid$.duration) |
      data_valid$.duration < 0

    if (any(bad_duration)) {
      bad_rows <- data_valid$.row_id[bad_duration]
