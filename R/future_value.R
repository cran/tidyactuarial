...
#'
#' Computes the future value of a payment invested at time 0 and accumulated
#' to a given time, using the annual effective interest rate implied by the
...
#'
#' @param amount Numeric vector of initial payment amounts.
...
#' @param time Numeric vector of times in years from valuation to accumulation.
#' @param output Character string. Use \code{"value"} to return a numeric
#'   future value, or \code{"table"} to return a tibble with intermediate
#'   calculations.
#'
#' @return
#' If \code{output = "value"}, a numeric vector of future values.
#'
...
future_value <- function(
  ...
  accumulation_factor <- (1 + i_effective)^time
  ...
  future_value_out[ok] <- amount[ok] * accumulation_factor[ok]
